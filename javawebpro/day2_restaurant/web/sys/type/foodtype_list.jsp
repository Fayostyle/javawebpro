		...
		<table class="MainArea_Content" align="center" cellspacing="0" cellpadding="0">
			...
			<tbody id="TableData">
				<c:choose>
					<c:when test="${not empty requestScope.listFoodType}">
						<c:forEach var="foodType" items="${requestScope.listFoodType}">
							<tr>
								...
								<td>
									<a href="${pageContext.request.contextPath}/foodType?id=${foodType.id}&method=viewUpdate"
									   class="FunctionButton">更新</a>
									<a href="${pageContext.request.contextPath}/foodType?id=${foodType.id}&method=delete"
					...
					<c:otherwise>
						<tr>
							<td colspan="3">没有你要找的数据，请保存记录再查看！</td>
						</tr>
					</c:otherwise>
				</c:choose>
			</tbody>
